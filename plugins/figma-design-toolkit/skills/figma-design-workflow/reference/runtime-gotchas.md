...
| Spacing variable looked up by name (`spacing/16` for 16px) | Names often follow a rem scale (`spacing/N = N×4px`) → `spacing/16` is 64px; missing names → `undefined` → silent skip → unbound literal | Map by resolved value: `spPx(px)` (see "Binding FLOAT variables" in `tokens-and-styles.md`); never `find(v=>v.name==='spacing/'+px)` |
...
| Instance-ratio / token audit descending into instances | Counts DS components' internal frames (NavRail, AppTopBar) as violations → false 70% where it's really 100% | Treat each INSTANCE as **opaque** — count it, don't recurse into it (see `audits.md`; manual `walk` that `continue`s on `type==='INSTANCE'`) |
