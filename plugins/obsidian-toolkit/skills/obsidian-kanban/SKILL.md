...
- **Done** — zrobione, karta **opisana rezultatem**: sekcja `## Rezultat` (co zrobione + link do artefaktu/PR/Figma). Przy przejściu na Done stempluj **`done_at: YYYY-MM-DD`**. Kandydat do **promocji do vaultu** (`obsidian-capture`). **Auto-archiwum:** karta Done starsza niż **30 dni** (od `done_at`) jest przenoszona do folderu archiwum przez `kanban-archive.sh` — kanał: zadanie `kanban-archive-done` w `scheduled-tasks` aplikacji Claude, **działa i zmierzone** (patrz operacja G); NIE LaunchAgent. Archiwizacja nieniszcząca (`mv`), notatka żyje. **Nie kasuj.**
...
- **Kanał, który działa: zadanie w `scheduled-tasks` aplikacji Claude** (nie launchd, nie crontab).
  Aplikacja ma dostęp do dysku, więc skrypt czyta vault i przenosi pliki normalnie. Na maszynie
  Piotra: task `kanban-archive-done`, codziennie 9:00, jedna komenda + raport (`SKILL.md` taska
  trzyma pełną ścieżkę boardu). **Zmierzone jako działające 2026-08-26** — dwa runy przeniosły
  realne karty: **72** karty w commicie `663312c` (2026-08-17 11:09) i **35** w `ff72428`
  (2026-08-24 11:09), oba minuty po slocie taska; historia gita vaultu jest tu mocniejszym
  dowodem niż log runu, bo pokazuje przeniesione pliki, nie sam przebieg.
- **⚠️ LaunchAgent NIE JEST działającym kanałem dla vaultu w `~/Documents` (zmierzone 2026-07-31).**
  ...
  `com.piotr.kanban-archive.plist` był załadowany i odpalał się codziennie, a jego log zawierał
  wyłącznie `Operation not permitted` — czyli **przez cały ten czas nie zrobił nic**, wyglądając
  na skonfigurowany. To ta sama klasa co „martwy check": zaplanowany automat bez sprawdzonego
  logu jest nieodróżnialny od działającego-cichego. Plist zdjęty; **nie odtwarzaj tej ścieżki.**
  Alternatywa „Full Disk Access dla tego, co odpala skrypt" jest szerokim uprawnieniem systemowym
  i wymaga świadomej zgody Piotra — nie jest cichym obejściem do wyboru przez agenta.
  **Zakładasz automat na vault → sprawdź LOG (albo git vaultu) po pierwszym odpaleniu, zanim
  uznasz go za zrobiony.**
- **Duża kolumna `Done` NIE jest sama w sobie dowodem, że automat stoi.** Dry-run
  (`KANBAN_ARCHIVE_DRYRUN=1`) rozstrzyga to w sekundę: 2026-08-26 board miał **233** karty Done,
  z czego **0 kwalifikowało się** do archiwizacji (221 młodszych niż 30 dni, 12 bez `done_at`
  do stemplowania). Zanim zdiagnozujesz automat jako zepsuty — odpal dry-run i porównaj
  „ile Done" z „ile eligible"; przy ~7 kartach domykanych dziennie kolumna Done rzędu 200+
  jest stanem normalnym, nie awarią.
...
- **`#` w NAZWIE PLIKU karty = zakaz** (i szerzej `^ [ ] |` — znaki, których Obsidian nie dopuszcza w nazwach notatek, bo łamią linki i anchory). Numer PR/issue zapisuj jako `(po PR 131)`, nie `(po #131)`. Powód nie jest kosmetyczny: ścieżka z `#` **wpisana bez cudzysłowów** do `cardOrders` w `.base` to dla YAML komentarz — `- KANBAN/Karta (po #131).md` parsuje się jako `KANBAN/Karta (po`, więc kolejny czytający produkuje kartę-widmo pod uciętą nazwą. Zmierzone 2026-07-31 na boardzie Manta: jedna karta wystąpiła jako **3 pliki** — realny `(po #131).md`, stub `(po.md` (22 B, sam `status:` → drugi kafel w To-do) i `(po 1.md` (0 B, bez frontmattera → kolumna Uncategorized); w tym samym commicie backupu vaultu doszedł niecytowany wpis w `.base`. Napotkasz `#` w nazwie → przemianuj kartę (`mv`) i popraw wpis w `.base`, nie tylko posprzątaj duplikaty. Wpis w `.base` pisany skryptem musi być **cytowany**, gdy nazwa nie jest plain-safe (`kanban-place-card.sh` to robi; eval 009).
  - **Nazwę karty przepuść przez `kanban-card-name.sh` PRZED utworzeniem pliku** (ścieżka absolutna, cross-project: `/Users/piotrkozlowski/Documents/piotr-toolkit/plugins/obsidian-toolkit/scripts/kanban-card-name.sh`) — `kanban-card-name.sh sanitize "<tytuł>"` zwraca bezpieczną nazwę (`#488` → `PR 488`), `check` zwraca exit 2 na niebezpiecznej. `kanban-place-card.sh` **odmawia** umieszczenia karty o niebezpiecznej nazwie (exit 1 + gotowa komenda `mv`), zamiast obejść problem cytowaniem. **Why:** sama reguła nie wystarczyła — 2026-08-25 powstała karta `Ogon po #488 …` **z poprawnie cytowanym** wpisem w `.base`, a i tak dała 0-bajtową kartę-widmo `Ogon po.md` i wikilink, który Obsidian czytał jako notatkę „Ogon po " + anchor „488 …". Cytowanie ratuje `.base`, nie nazwę — dwa z trzech objawów siedzą poza tym plikiem.
